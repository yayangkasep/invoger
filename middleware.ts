...
function getTokenFromReq(req: Request | import('next/server').NextRequest) {
  const cookieToken = (req as any).cookies?.get?.(AUTH_TOKEN_KEY)?.value ?? undefined
...
export async function middleware(req: NextRequest) {
  ...
  const url = req.nextUrl.clone()
    ...
    try {
      ...
      const envHasApiKey = Boolean(process.env.FIREBASE_API_KEY || process.env.NEXT_PUBLIC_FIREBASE_API_KEY)
      // eslint-disable-next-line no-console
      console.debug('[middleware] pathname=', pathname, 'tokenSnippet=', tokenSnippet, 'envHasApiKey=', envHasApiKey)
    } catch (_) {}
