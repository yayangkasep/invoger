...
// columns are built inside the component so actions can access component state (open dialog)
...
export function ProductsTable({
  ...
  const displayLoading =
    typeof loadingProp === "boolean" ? loadingProp : loading;

  React.useEffect(() => {
    // Do not generate uuid client-side. Use server-provided `uuid` when available.
    // If a product doesn't have a server uuid yet, keep it undefined so it can be backfilled server-side.
    setData(initialData || []);
  }, [initialData]);
  ...
  // Open dialogs if URL params present on mount/update
  React.useEffect(() => {
    const idParam = searchParams?.get?.("ID") ?? null
    if (idParam) {
      const found = data.find((d) => d.uuid === idParam || d.id === idParam)
      if (found) {
        // if the delete dialog is already open for this id, keep it open
        if (deleteOpenId === found.id) {
          // nothing to do, delete already open
        } else {
          // otherwise open edit dialog by default
            ...
            <div className="flex justify-center">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-32">
                  ...
                  {/* Make Delete behave like Edit: open via DropdownMenuItem onClick and keep dialog controlled */}
                  <DropdownMenuItem
                    onClick={() => {
                      setDeleteOpenId(row.original.id)
                      // update URL with ID param (same as edit)
                      const params = new URLSearchParams(Array.from(searchParams || []));
                      params.set('ID', row.original.uuid || row.original.id);
                      router.replace(`${location.pathname}?${params.toString()}`);
                    }}
              ...
              {/* Render the Delete dialog outside of the dropdown so it doesn't get hidden/unmounted when the menu closes */}
                  {/* per-row dialog removed: using shared DeleteProductDialog instance */}
            </div>
          ),
        },
      ],
    ...
    <div className="w-full">
      ...
      <EditProductDialog
        ...
        onUpdate={(updated) => {
          setData((prev) =>
            prev.map((r) =>
              r.id === updated.id ? { ...r, ...(updated as any) } : r
            )
          );
          setEditOpen(false);
        }}
      />
      {/* Single shared Delete dialog instance */}
      <DeleteProductDialog
        id={deleteOpenId ?? ''}
