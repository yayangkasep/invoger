...
export default function AddProductDialog({
  ...
  onAdd?: (payload: (ProductPayload | Partial<Product>) & { __tempId?: string }) => Promise<void> | void
  open?: boolean
  onOpenChange?: (open: boolean) => void
  onAddSuccessReplace?: (tempId: string, realProduct: ProductPayload & { id: string }) => void
  onProductAdded?: (prod: ProductPayload & { id: string }) => void
}) {
  ...
  const [errors, setErrors] = React.useState<{ Name?: string; Code?: string; Price?: string }>({})
  const [touched, setTouched] = React.useState<{ Name?: boolean; Code?: boolean; Price?: boolean }>({})
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const payload = { Name: name || "", Code: code || "", Price: Number(cleaned || 0), Promotion: promotion || "None", __tempId: tempId }
    onAdd?.(payload)
    // also persist to Firestore
    ;(async () => {
      try {
        setLoading(true)
        // Frontend-only: simulate a successful persist and return a mock id
        ...
        if (onAddSuccessReplace) {
          ...
          if (maybeTemp) onAddSuccessReplace(maybeTemp, { ...payload, id: id ?? `mock-${Date.now()}` })
        }
        // inform parent directly (optional) so it can refresh/merge if needed
        onProductAdded?.({ ...payload, id })
        // auto-close after ~2s, then hide alert after ~2.5s
  ...
  trigger={<Button><IconFilePlus className="size-4" />Add Products</Button>}
      footer={
        <>
          <DialogClose asChild>
            <Button variant="outline">Cancel</Button>
          </DialogClose>
          <Button onClick={() => formRef.current?.requestSubmit()} disabled={loading || formInvalid}>{loading ? "Saving..." : "Save"}</Button>
        </>
      }
    >
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-3">
        ...
        <div>
          ...
          <Input
            ...
            onBlur={() => { setTouched((s) => ({ ...s, Price: true })); if (!cleanNumericInput(price)) setErrors((s) => ({ ...s, Price: 'Required' })); else setErrors((s) => ({ ...s, Price: undefined })); }}
            className="w-full"
