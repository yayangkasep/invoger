  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const message =
          body?.message || body?.error || `Import failed: ${res.status}`;
    ...
    >
      <div className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          CSV format: Name,Code,Price (header required, order: Name,Code,Price)
        </p>
        {file ? (
        ...
        {result ? (
          <pre className="mt-2 p-2 text-xs bg-surface rounded-md overflow-auto">
            {result}
          </pre>
        ) : null}
      </div>
