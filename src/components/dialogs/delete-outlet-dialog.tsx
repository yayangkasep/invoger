...
export default function DeleteOutletDialog({
  id,
  onDeleted,
  trigger,
  open: openProp,
  ...
  function renderTrigger() {
    if (!trigger) {
      return (
        <button className="text-destructive" onClick={() => handleOpenChange(true)}>Delete</button>
      )
    }
  ...
  return (
    <AlertDialog open={controlledOpen} onOpenChange={handleOpenChange}>
      ...
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel disabled={loading} onClick={() => handleOpenChange(false)}>Cancel</AlertDialogCancel>
