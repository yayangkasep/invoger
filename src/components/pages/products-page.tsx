...
export default function ProductsPageComponent() {
  ...
  async function loadProducts() {
    ...
    try {
      ...
      const normalized: Product[] = items.map((o: any) => ({
        ...
        Code: (o.code as string) ?? (o.Code as string) ?? (o.sku as string) ?? "",
        Price: typeof o.price === 'number' ? o.price : typeof o.Price === 'number' ? o.Price : Number(o.Price ?? o.price ?? 0),
        Promotion: (o.promotion as string) ?? (o.Promotion as string) ?? (o.status as string) ?? 'None',
      }))
      setProducts(normalized)
    } catch (err: unknown) {
      // eslint-disable-next-line no-console
      console.error('Failed to load products', err)
      const msg = err && typeof err === 'object' && 'message' in err ? String((err as { message?: unknown }).message) : 'Failed to load products'
      toast.error(msg)
    } finally {
      setLoading(false)
    }
  ...
  const createProduct = async (p: { name: string; price?: number; description?: string; meta?: any }) => {
    try {
      ...
      if (prod) {
        const normalized: Product = {
          ...
          Price: typeof prod.Price === 'number' ? prod.Price : Number(prod.Price ?? prod.price ?? 0),
    ...
    <div>
      <ResourcePage
        ...
        onSearch={(q) => setQuery(q)}
          toolbarActions={[
          { label: "Import", node: <ImportDialog onImport={() => loadProducts()} /> },
          {
            label: "Add Product",
            node: (
              <AddProductDialog
