...
import { setToken as setClientToken } from '@/lib/auth/cilent'
  ...
  const payload: unknown = await res.json().catch(() => ({}))
          ...
          if (jsonStart !== -1) {
            try {
              ...
              const parsedMessage =
                ...
                (maybeErrors && maybeErrors.length > 0 && typeof (maybeErrors[0] as Record<string, unknown>).message === 'string' && ((maybeErrors[0] as Record<string, unknown>).message as string)) ||
                undefined
      ...
      if (idToken) {
        try {
          ...
          document.cookie = `invoger_token=${idToken};path=/;SameSite=Lax${expires}`
        } catch (e) {
