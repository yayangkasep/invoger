...
import { AUTH_TOKEN_KEY, AUTH_EXPIRES_IN, AUTH_FORCE_EXPIRES_IN } from "@/lib/auth/constants";
...
export async function POST(req: Request) {
  ...
  maxAge: AUTH_FORCE_EXPIRES_IN ? parseInt(AUTH_EXPIRES_IN, 10) : (res?.expiresIn ? parseInt(res.expiresIn, 10) : parseInt(AUTH_EXPIRES_IN, 10)),
    };
    ...
    const response = NextResponse.json({
      ...
      localId: res.localId,
      idToken: res.idToken,
  expiresIn: AUTH_FORCE_EXPIRES_IN ? AUTH_EXPIRES_IN : (res.expiresIn ?? AUTH_EXPIRES_IN),
    });
