...
export async function POST(req: Request) {
  try {
    ...
    try { await verifyIdToken(token) } catch (e) { return NextResponse.json({ error: 'Unauthorized' }, { status: 401 }) }

    const body = await req.json().catch(() => ({}))
    const parsed = EditOutletSchema.safeParse(body)
    if (!parsed.success) return NextResponse.json({ error: 'validation_error', issues: parsed.error.format() }, { status: 400 })
    ...
    if (!snap.exists) {
      const uuidSnap = await db.collection('outlets').where('uuid', '==', id).limit(1).get()
      if (uuidSnap.empty) return NextResponse.json({ error: 'not_found' }, { status: 404 })
    ...
    return NextResponse.json({ ok: true, outlet: { id: updated.id, ...updated.data() } })
  } catch (err: any) {
