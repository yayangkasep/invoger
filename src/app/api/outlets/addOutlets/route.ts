...
export async function POST(req: Request) {
	try {
		...
		const existingSnap = await db.collection('outlets').where('code', '==', code).limit(1).get()
		if (!existingSnap.empty) {
			...
			if (
				...
				return NextResponse.json({ ok: true, skipped: true, id: doc.id, outlet: { id: doc.id, ...data } })
			}
		...
		return NextResponse.json({ ok: true, id: docRef.id, outlet: { id: created.id, ...created.data() } })
	} catch (err: any) {
