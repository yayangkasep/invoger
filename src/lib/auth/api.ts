...
export async function loginWithEmailPassword(email: string, password: string): Promise<LoginResponse> {
	...
	if (!res.ok) {
		...
		throw new Error(`Firebase login failed: ${res.status} ${res.statusText} ${short}`)
	}
...
export async function getProducts(apiUrl = '/api/products', idToken?: string) {
	...
	const res = await fetch(apiUrl, { headers, credentials: idToken ? undefined : 'same-origin' })
	if (!res.ok) throw new Error(`getProducts failed: ${res.status} ${res.statusText}`)
	return res.json()
}
...
export async function getOutlets(apiUrl = '/api/outlets', idToken?: string) {
	...
	const res = await fetch(apiUrl, { headers, credentials: idToken ? undefined : 'same-origin' })
	if (!res.ok) throw new Error(`getOutlets failed: ${res.status} ${res.statusText}`)
	return res.json()
}
